...
CREATE POLICY IF NOT EXISTS "Anyone can read geocode cache"
  ON geocode_cache
  FOR SELECT
  USING (true);

-- Allow authenticated users to insert and update cache
CREATE POLICY IF NOT EXISTS "Authenticated users can insert geocode cache"
  ON geocode_cache
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can update geocode cache"
  ON geocode_cache
  FOR UPDATE
  TO authenticated
  USING (true)
